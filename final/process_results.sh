...
for g in $(echo "$victims"); do cut -d, -f-5 all_results.csv | grep ",${g},failure\|,${g},error$"; done | grep "^.*,pass,"  > pv-info.csv

cut -d, -f1 pv-info.csv | cut -d'[' -f1 | sort -u > new-polluters.txt
